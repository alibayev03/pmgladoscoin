...
$(document).ready(() => {	
                ...
                <div style="width: 100%; border-radius: 10px; background: #4a89dc; padding: 10px; margin-bottom: 10px;">
                    <div class="row" style="margin:0; padding:0;">
                        ...
                        <div class="col-8" style="margin:0; padding:0">
                            <div class="content">
                                <p class="mb-n1 color-white opacity-50 font-600">${formattedLeaderBalance} GLD</p>
    ...
    $.post('/api/interface/affilate.php', { user_id: user.id }, (response) => {
        ...
        $('#referral-link-text').text(`https://t.me/GLDcoinbot?start=${data.ref_str}`);
        $('#referral-income').text(`${parseInt(data.referal_income).toLocaleString('ru-RU')} GLD`);
                ...
                const listItem = `
                    <div class="card card-style bg-blue-dark">
                        <div class="content">
                            ...
                            <p class="color-white">
                                Баланс друга: <strong>${parseInt(ref.balance).toLocaleString('ru-RU')} GLD</strong><br>
                                Доход друга за сутки: <strong>${parseInt(ref.daily_income_f).toLocaleString('ru-RU')} GLD</strong><br>
                                Твой доход за сутки: <strong>${parseInt(ref.daily_income).toLocaleString('ru-RU')} GLD</strong>
                            </p>
                        </div>
                    </div>`;
    ...
    const endGame = () => {
        ...
        if (gameStart == 1) {
            ...
            const GLDEarned = (score * fallGameRevenue).toLocaleString('ru-RU');
            ...
            recordText = `https://t.me/GLDcoinbot\nУ меня ${score} очков в "Поймай мерзавчик"! Сможешь лучше?`;

            Swal.fire({
                icon: "success",
                title: "Отлично!",
                html: `Собрано кошельков: ${score}<br>Вы заработали ${GLDEarned} GLD`,
    ...
    $(document).on('click', '.booster-card', function (e) {
        ...
        Swal.fire({
            ...
            html: `
                ...
                <p><strong>Стоимость улучшения:</strong> ${typeof boosterPrice === 'number' ? boosterPrice.toLocaleString('ru-RU') : boosterPrice} GLD</p>
            `,
